...
export default function Room() {
  ...
  useEffect(() => {
    ...
    sessionStorage.setItem(
      `room_${id}_form_individualAmount`,
      individualAmount,
    );
  ...
  const addParticipant = async () => {
    if (isLocked) {
      toast.error(
        "Комната защищена паролем. Нажмите замочек для разблокировки.",
      );
      return;
    }
        ...
        {participants.length === 0 ? (
          <div className="empty-state">
            ...
            <div className="empty-subtitle">
              Добавьте первого, чтобы начать делить счёт
            </div>
          </div>
        ) : (
        ...
        <>
          <div className="card">
            ...
            <div className="form-row" style={{ marginTop: "12px" }}>
              ...
              <button className="btn-primary" onClick={addToParticipant}>
                Добавить
              </button>
            </div>
          ...
          <div className="card">
            ...
            <div className="form-row">
              ...
              <div className="form-group">
                ...
                <select
                  value={sharedPayerId}
                  onChange={(e) => setSharedPayerId(e.target.value)}
                >
                  <option value="">Выберите плательщика</option>
                  {participants.map((p) => (
                    <option key={p.id} value={p.id}>
      ...
      <div className="card">
        ...
        {logs.length === 0 ? (
          <div className="empty-state">
            ...
            <div className="empty-subtitle">
              Начислите или распределите сумму — записи появятся здесь
            </div>
          </div>
        ) : (
          <div className="logs-modern">
            {logs.map((log) => {
              ...
              return (
                <div
                  key={log.id}
                  className={`log-card ${log.type} ${log.is_reverted ? "reverted" : ""}`}
                >
                  <div className="log-card-header">
                    <div className="log-card-meta">
                      ...
                      <span className="log-date">
                        {dateStr} · {timeStr}
                        {log.is_reverted && (
                          <span className="reverted-label"> · Отменено</span>
                        )}
                      </span>
                    </div>
                    {!isLocked &&
                      !log.is_reverted &&
                      (log.type === "individual" || log.type === "shared") && (
                        <button
                  ...
                  <div className="log-card-body">
                    ...
                    {log.payer_name && (
                      <div className="log-payer">
                        <span className="log-label">Оплатил:</span>{" "}
                        {log.payer_name}
                      </div>
                    )}
                    {log.note && (
                      <div className="log-note-modern">
                        <span className="log-label">Комментарий:</span>{" "}
                        {log.note}
                      </div>
                    )}
                  </div>
