...
export default function SharedExpenseForm({ 
  participants, sharedAmount, setSharedAmount, sharedPayerId, setSharedPayerId, 
  sharedNote, setSharedNote, selectedIds, toggleSelectedId, selectAll, deselectAll, 
  sharedPreview, distributeShared 
}: Props) {
  return (
    <div className="card" style={{ padding: "16px" }}>
      ...
      <div className="form-row" style={{ gap: "10px" }}>
        <div className="form-group">
          <label style={{ fontSize: "0.8rem" }}>Сумма для распределения</label>
          <input 
            type="number" 
            value={sharedAmount} 
        ...
        <div className="form-group">
          <label style={{ fontSize: "0.8rem" }}>Кто платил</label>
          <select 
            value={sharedPayerId} 
      ...
      <div className="form-group" style={{ marginTop: "10px" }}>
        <label style={{ fontSize: "0.8rem" }}>Примечание</label>
        <input 
          type="text" 
          value={sharedNote} 
          onChange={(e) => setSharedNote(e.target.value)} 
          placeholder="Например: Общий чек" 
        ...
        {participants.map(p => (
          ...
          >
            <input 
              ...
              className="mr-2"
              style={{
                accentColor: 'var(--checkbox-accent)', // ✅ Цвет галочки
                backgroundColor: 'var(--bg-input)',    // ✅ Фон чекбокса
                borderColor: 'var(--border)',          // ✅ Граница
                width: '18px',
                height: '18px',
                cursor: 'pointer',
                borderRadius: '4px',
      ...
      {sharedPreview && (
        <div className="preview-box" style={{ marginTop: "10px", padding: "10px" }}>
          ...
          {sharedPreview.map((item, idx) => (
            <div key={idx} className="preview-row" style={{ fontSize: "0.85rem", padding: "3px 0" }}>
              <span>{item.name}</span>
