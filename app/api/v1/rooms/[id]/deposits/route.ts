...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const { participantId, amount, isAdvance, note } = schema.parse(json);

    await db.insert(deposits).values({ roomId, participantId, amount, isAdvance, note });
    revalidateTag(`settlements-${roomId}`); // Инвалидация переводов
