...
export default function RoomClient({ initialData, roomId }: { initialData: Room, roomId: string }) {
  const { data: roomData, mutate } = useSWR(`/api/v1/rooms/${roomId}`, fetcher, {
  ...
  const addParticipant = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Сохраняем ключ нового участника, если мы его добавили (мы админ, но можем хотеть войти как он)
        // Или просто обновляем список
        sessionStorage.setItem(`participantKey_${roomId}`, data.participantKey);
        setMyParticipantKey(data.participantKey); // Auto-login as new participant? Optional.
    ...
    <div className="min-h-screen p-4 bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 font-sans">
      <header className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex gap-2">
           ...
           <button onClick={() => window.location.reload()} className="px-3 py-1 bg-gray-200 dark:bg-gray-700 rounded text-sm">Refresh</button>
        </div>
      </header>
